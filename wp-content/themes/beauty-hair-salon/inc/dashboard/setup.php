...
<div class="theme-offer">
  ...
  if(isset($_GET['import-demo'])){
    ...
    $service_title=array('Spa Express','Hair Treatment','Aroma Therapy','Body Relaxation');
    ...
    set_theme_mod( 'beauty_hair_salon_shop_text', 'Best We Can Give Through Our Products' );

    set_theme_mod( 'beauty_hair_salon_shop_title', 'Our Best Products' );

    set_theme_mod( 'beauty_hair_salon_shop_number', '4' );

    $prodcut_title=array('Pink Color Nail Paint','Natural Eye Liner','Natural Vitamin C','Handmade Body Soap');

    $product_price=array('$8.50','$12','$10.50','$8.50');

    wp_insert_term(
      'Best_Products', // the term
    ...
    if ( class_exists( 'WooCommerce' ) ) {
      for($i=1;$i<=4;$i++) {
        ...
        $term = get_term_by('name', 'Best_Products', 'product_cat');
        wp_set_object_terms($post_id, $term->term_id, 'product_cat');

        update_post_meta( $post_id, '_price', $product_price[$i-1] );
        update_post_meta( $post_id, '_regular_price', $product_price[$i-1] );
      

        // Fetch image and upload
        $image_url = get_stylesheet_directory_uri() . '/assets/product'.$i.'.jpg';
    ...
    set_theme_mod( 'beauty_hair_salon_shop_category', 'Best_Products' );

  } ?>
  <p class="plugin-text"><?php echo esc_html_e('Before Demo Import first install given plugin, ','beauty-hair-salon'); ?><span><?php echo esc_html_e('WooCommerce','beauty-hair-salon'); ?></span></p>
  <p class="note"><?php esc_html_e("If your website is already live and containing data, please make a backup.This importer will override the Beauty Hair Salon's new customizable values.",'beauty-salon-spa'); ?></p>
